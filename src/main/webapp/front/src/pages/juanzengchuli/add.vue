...
<div :style='{"width":"1000px","padding":"20px","margin":"10px auto","position":"relative","background":"#D8D8D8"}'>
    <el-form
      ...
      label-width="110px"
    >
          <el-form-item :style='{"width":"40%","padding":"10px","margin":"0 2% 10px","display":"inline-block"}' label="处理编号" prop="chulibianhao">
          ...
          <el-form-item :style='{"width":"40%","padding":"10px","margin":"0 2% 10px","display":"inline-block"}' label="处理方式" prop="chulifangshi">
            <el-input v-model="ruleForm.chulifangshi" 
                placeholder="处理方式" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"40%","padding":"10px","margin":"0 2% 10px","display":"inline-block"}' label="处理时间" prop="chulishijian" >
              <el-date-picker
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('juanzengchuli/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('juanzengchuli/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('juanzengchuli/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
