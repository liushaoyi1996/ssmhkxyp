  ...
  export default {
    data() {
      return {
        ...
        rules: {
          shenlingzhanghao: [
            { required: true, message: '申领账号不能为空', trigger: 'blur' },
          ],
          mima: [
            { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          shenlingxingming: [
            { required: true, message: '申领姓名不能为空', trigger: 'blur' },
          ],
          touxiang: [
          ],
          xingbie: [
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('shenlingzhe/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('shenlingzhe/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('shenlingzhe/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
